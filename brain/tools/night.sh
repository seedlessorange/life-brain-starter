...
#
# On a Mac, scheduled by ~/Library/LaunchAgents/com.lifebrain.night.plist
# (run `brain/tools/setup_night.sh` once to install it), which runs it under
# zsh. On Linux (a server, a Docker cron) the same file runs under bash or
# sh — it sticks to the POSIX subset all three shells share, and the
# Mac-only helpers (pmset, caffeinate) step aside when absent.
...
# battery at all, which pmset prints as "AC Power" — so they always pass, and
# so does any machine without pmset (Linux boxes are plugged in).
...
# caffeinate (Macs only): hold the machine awake for the run's real few
# minutes rather than letting it doze mid-job and crawl (same fix as
# morning.sh). On Linux $CAFF stays empty and the line runs claude directly.
CAFF=""
...
for JOB in $NIGHT_JOBS; do
  ...
  env -u ANTHROPIC_API_KEY -u ANTHROPIC_AUTH_TOKEN LIFEBRAIN_UNATTENDED=1 \
    $CAFF claude -p "/$JOB" --permission-mode bypassPermissions \
