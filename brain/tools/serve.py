...
import grab as GRAB    # noqa: E402  (recent downloads, and the limits on what may come in)
...
# A container (or any box behind NAT) has no choice but to bind 0.0.0.0,
# which used to mean "guard off". BRAIN_ALLOWED_HOSTS — a comma-separated
# list of the names the owner actually reaches the page by — keeps the guard
# ON in that case: only those names pass, everything else is treated as a
# foreign page.
_EXTRA_HOSTS = {_host_only(h) for h in
                os.environ.get("BRAIN_ALLOWED_HOSTS", "").split(",") if h.strip()}
if _EXTRA_HOSTS:
    ALLOWED_HOSTS.update(_EXTRA_HOSTS)
    GUARD_ENFORCED = True
...
def request_is_own(host_header, origin_header, allowed):
    ...
    "null" counts as another site: browsers send it for sandboxed iframes and
    file:// pages, and the served page is never either of those.
    """
    if host_header and _host_only(host_header) not in allowed:
        return False
    o = (origin_header or "").strip()
    if o and (o.lower() == "null" or _host_only(o) not in allowed):
        return False
    return True
...
def tailnet_dns_name():
    """This machine's ts.net name, or None. `tailscale serve` fronts the page
    under that name (with real HTTPS — what lets a phone install the app), and
    requests proxied through it can carry it in Host, so the guard must know it."""
    for exe in ("/Applications/Tailscale.app/Contents/MacOS/Tailscale",
                "tailscale",
                r"C:\Program Files\Tailscale\tailscale.exe"):
        try:
        ...
        if r.returncode != 0:
            continue
        try:
            name = (json.loads(r.stdout).get("Self") or {}).get("DNSName") or ""
        except Exception:
            continue
...
# The list itself lives in usage.py, next to what each one costs.
MODELS = set(usage.PICKABLE)
...
# Defined in grab.py, which is the other door into brain/files/ — two doors
# with different locks is how a limit gets quietly bypassed.
MAX_IMAGE_BYTES = GRAB.MAX_IMAGE_BYTES
MAX_DOC_BYTES = GRAB.MAX_DOC_BYTES
MAX_ATTACHMENTS = GRAB.MAX_ATTACHMENTS
IMAGE_EXT = GRAB.IMAGE_EXT
SAFE_EXT = GRAB.SAFE_EXT
...
def save_files(files):
    ...
    for f in files[:MAX_ATTACHMENTS]:
        ...
        with open(target, "wb") as fh:
            fh.write(blob)
        # .docx arrives with a text twin — see grab.docx_twin for why.
        saved.append(os.path.relpath(GRAB.docx_twin(target), BRAIN))
    return saved
...
_MODEL_TIER = {"haiku": 0, "sonnet": 1, "opus": 2, "fable": 3}
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.startswith("/enablebanking/callback"):
            ...
            self.wfile.write(body)
            return
        # Everything this handler serves IS the brain — the rendered pages,
        # people.md, config.json — so a rebinding page must not reach any of
        # it. Only the bank callback above skips the guard: the bank's
        # redirect is a top-level navigation under our own Host anyway.
        if not self._guard():
            return
        ...
        if self.path.startswith("/api/downloads"):
            # What just landed in ~/Downloads, so a document that arrived by
            # mail reaches an ask without a trip through a file picker.
            from urllib.parse import urlparse, parse_qs
            q = parse_qs(urlparse(self.path).query)
            try:
                hours = float((q.get("hours") or ["48"])[0])
            except ValueError:
                hours = 48
            return self._json({"files": GRAB.recent(hours), "folder": GRAB.DOWNLOADS})
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path == "/api/downloads/take":
                # Copies into brain/files/<today>/ — the originals stay in
                # Downloads, because the brain does not write outside itself.
                return self._json({"ok": True,
                                   "saved": GRAB.take(body.get("names", []))})
            ...
            if self.path == "/api/tour":
                # Whether she has been shown around, kept here rather than in
                # the browser so one dismissal answers for every page, every
                # address and every device. {done: false} re-arms it.
                want = bool(body.get("done", True))
                cfg = load_json(os.path.join(BRAIN, "config.json"), {})
                if bool(cfg.get("tour_done")) != want:
                    _config_set("tour_done", want)
                    rebuild(map_too=True)
                return self._json({"ok": True, "tour_done": want})
            ...
            if self.path == "/api/email/check":
                ...
                rebuild(map_too=False)
                return self._json(r)
            if self.path == "/api/mail/tasks/check":
                # Bodies, but only whitelisted+DMARC senders, only her click,
                # and only into a no-tools reader (see mail_tasks.py).
                import mail_tasks
                r = mail_tasks.check(days=body.get("days"))
                rebuild(map_too=False)
                return self._json(r)
            if self.path == "/api/mail/tasks/act":
                import mail_tasks
                r = mail_tasks.act((body.get("id") or "").strip(),
                                   (body.get("action") or "").strip())
                rebuild(map_too=False)
                return self._json(r)
            if self.path == "/api/mail/tasks/senders":
                import mail_tasks
                cur = mail_tasks.set_senders(add=body.get("add", ""),
                                             remove=body.get("remove", ""))
                rebuild(map_too=False)
                return self._json({"ok": True, "senders": cur})
...
def main():
    ...
    # is what keeps phone access working through the guard. The ts.net name is
    # allowed too, for phones coming through `tailscale serve`.
    ALLOWED_HOSTS.update(_host_only(a) for a in binds)
    ts_name = tailnet_dns_name()
    if ts_name:
        ALLOWED_HOSTS.add(ts_name)
    servers = []
    for addr in binds:
        try:
