...
def build():
    ...
    if today_md.strip() or habits:
        ...
        V["today"].append(_mailtasks_tray())
        _pstamp = _plan_time()
...
.mtdue{color:var(--terra)}
.mttray{margin:0 0 var(--s4)}
.mttray h2{margin:0 0 8px}
.mtrow{display:flex;align-items:baseline;gap:8px;padding:7px 0;
  border-top:1px solid var(--line)}
.mtrow:first-of-type{border-top:0}
.mttask{flex:1;min-width:0;font-size:var(--t-sm)}
.mttask i{display:block;color:var(--faint);font-style:normal;
  font-size:var(--t-xs);margin-top:2px;overflow:hidden;text-overflow:ellipsis;
  white-space:nowrap}
.mtmore{margin:6px 0 0;font-size:var(--t-xs);color:var(--faint)}
.mtchip{display:inline-block;border:1px solid var(--line);border-radius:9px;
  padding:1px 7px;margin:0 4px 3px 0;font-size:var(--t-xs)}
.mtx{border:0;background:none;cursor:pointer;color:var(--dim);padding:0 0 0 4px}
...
(function(){
  ...
  function mtSay(m){ if(mtHelp) mtHelp.textContent = m; }
  ...
  document.querySelectorAll('[data-mtact]').forEach(function(b){
    b.onclick = function(){
      ...
      post('/api/mail/tasks/act', {id: b.getAttribute('data-mtid'),
                                   action: b.getAttribute('data-mtact')})
        .then(function(){ reloadWhenReady(); })
